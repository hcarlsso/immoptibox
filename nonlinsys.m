...
% Version 04.04.10.
% Bug fix by P. C. Hansen, DTU Compute, Nov. 15, 2017.
...
if  isempty(x0),  stop = -1;  x = [];
  ...
  if  ~stop
    ...
    if  ~stop
      %  Finish initialization
      % Per Chr. Hansen, DTU Compute:
      % The following line is incorrect; I replaced it with the one below.
      % opts = checkopts(opts, [.1*(1+norm(x,inf)) 1e-6 1e-8 100 1e-6]); 
      opts = checkopts('nonlinsys', [.1*(1+norm(x,inf)) 1e-6 1e-8 100 1e-6]); 
      % Jacobian
      sB = size(B0); 
