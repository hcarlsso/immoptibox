...
while ~stop
    if VERBOSE > 0
        fprintf("==================================\n")
        fprintf("Start step k=%d kit=%d\n", k, kit)
    end
    if VERBOSE > 0
        fprintf("Check gradient is ng = %.1e <= tolg = %.1e ? \n", ng, tolg)
    end
    if  ng <= tolg
        stop = 1;
        if VERBOSE > 0
            fprintf("Stop due to small gradient\n")
        end
    else
        [h, mu] = geth(A,g,mu);
        nh = norm(h);   
        nx = tolx + norm(x);
        if VERBOSE > 0
           fprintf("Check step length is nh = %.1e <= nx = %.1e * tolx = %.1e,  \n", nh, nx, tolx) 
        end
        if  nh <= tolx*nx
            stop = 2;
            if VERBOSE > 0
                fprintf("Stop due to small step length\n")
            end
        end
    end
    if  ~stop
        ...
        if  ~stop
            ...
            if VERBOSE > 0
                fprintf("Change in obj func df = %.1e\n", df)
            end
